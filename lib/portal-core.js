...
function getHeuristicProbability(match, odd, context, fallbackProbability) {
  if (odd.marketKey === "h2h") {
    return h2hProbability(match, odd, context);
  }

  if (odd.marketKey === "totals") {
    return totalsProbability(match, odd, context, fallbackProbability);
  }

  if (odd.marketKey === "spreads") {
    return spreadsProbability(match, odd, context, fallbackProbability);
  }
...
function h2hProbability(match, odd, context) {
  ...
  if (context.sportKey === "futbol") {
    ...
    if (odd.market.includes(match.home)) {
      return { probability: homeProbability, rationale: diff > 0 ? "Local fuerte con ventaja de cancha" : "Local equilibrado" };
    }
    if (odd.market.includes(match.away)) {
      return { probability: awayProbability, rationale: diff < 0 ? "Visitante competitivo" : "Visitante exigido" };
    }
    return { probability: drawProbability, rationale: "Empate dependiente de paridad" };
  }

  if (odd.market.includes(match.home)) {
    return { probability: homeRaw, rationale: diff > 0 ? "Local con mejor rating" : "Local sin ventaja clara" };
  }
...
function totalsProbability(match, odd, context, fallbackProbability) {
  ...
  if (odd.market.toLowerCase().includes("mas de")) {
    return { probability: overProbability, rationale: `Linea comparada con media ${baselineTotal}` };
  }

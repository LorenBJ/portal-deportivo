...
export async function fetchOddsApiFeed() {
  ...
  const results = await Promise.allSettled(config.sportKeys.map((sportKey) => fetchSportBundle(sportKey, config)));

  const successful = results.filter((result) => result.status === "fulfilled").map((result) => result.value);
  const matches = successful.flatMap((entry) => entry.matches).filter((match) => match.odds.length > 0).sort((left, right) => new Date(left.kickoff).getTime() - new Date(right.kickoff).getTime());
  const remaining = successful.map((entry) => entry.remaining).filter((value) => Number.isFinite(value));
  const errors = results.filter((result) => result.status === "rejected").map((result) => result.reason instanceof Error ? result.reason.message : "unknown_error");
  ...
  return {
    ...
    meta: {
      ...
      errors,
      reason: matches.length ? "ok" : successful.length ? "no_matches_returned" : "all_sports_failed"
    }
  };
}
...
async function fetchSportBundle(sportKey, config) {
  ...
  return {
    ...
    matches: oddsPayload.map((event) => normalizeEvent(event, scoresById.get(event.id))).filter(Boolean)
  };
}
...
function extractBestOutcomes(event) {
  ...
  for (const bookmaker of event.bookmakers ?? []) {
    for (const market of bookmaker.markets ?? []) {
      for (const outcome of market.outcomes ?? []) {
        ...
        const existing = grouped.get(key) ?? { id: `${event.id}-${key}`, market: label, price, bookmaker: bookmaker.title, marketKey: market.key, consensusSamples: [] };
  ...
  return Array.from(grouped.values()).map((entry) => {
    ...
    return { id: entry.id, market: entry.market, price: entry.price, bookmaker: entry.bookmaker, consensusProbability, modelProbability: applyHeuristicBoost(entry.marketKey, entry.market, consensusProbability, event) };
...
function applyHeuristicBoost(marketKey, marketLabel, consensusProbability, event) {
  ...
  if (marketKey === "spreads" && event.sport_key === "basketball_nba") adjustment += 0.012;
...
function formatScore(event, scoreEvent) {
  if (!scoreEvent?.scores?.length) return inferStatus(event.commence_time, scoreEvent) === "live" ? "En juego" : "-";

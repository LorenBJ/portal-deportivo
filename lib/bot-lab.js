...
export function buildAutoTickets(matches = [], settings = {}, existingTickets = []) {
  ...
  const dayFocus = settings.dayFocus ?? "today";
  ...
  const scopedCandidates = matches
    ...
    .filter((match) => matchMatchesDayFocus(match, dayFocus))
...
function matchMatchesDayFocus(match, dayFocus) {
  if (dayFocus === "week") return getRelativeDay(match.kickoff) >= 0 && getRelativeDay(match.kickoff) <= 6;
  const targetMap = { today: 0, tomorrow: 1, plus2: 2, plus3: 3 };
  const target = targetMap[dayFocus] ?? 0;
  return getRelativeDay(match.kickoff) === target;
}

function getRelativeDay(kickoff) {
  if (!kickoff) return Number.NaN;
  const todayKey = formatDayKey(new Date());
  const matchKey = formatDayKey(new Date(kickoff));
  if (!todayKey || !matchKey) return Number.NaN;
  const [todayYear, todayMonth, todayDay] = todayKey.split("-").map(Number);
  const [matchYear, matchMonth, matchDay] = matchKey.split("-").map(Number);
  const todayUtc = Date.UTC(todayYear, todayMonth - 1, todayDay);
  const matchUtc = Date.UTC(matchYear, matchMonth - 1, matchDay);
  return Math.round((matchUtc - todayUtc) / 86400000);
}

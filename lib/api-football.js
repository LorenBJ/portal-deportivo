...
export async function fetchFootballFeed() {
  ...
  const liveResult = await apiFetch(
    "/fixtures",
    { live: liveLeagueIds, timezone: config.timezone },
    config,
    "fixtures_live"
  ).catch((error) => ({ response: [], headers: {}, error }));

  const fixtureDateWindow = buildDateWindow(config.targetDate, config.fixtureDays);
  const leagueResults = [];
  const errors = liveResult.error
    ? [liveResult.error instanceof Error ? liveResult.error.message : String(liveResult.error)]
    : [];

  for (const league of config.leagues) {
    try {
      leagueResults.push(await fetchLeagueBundle(league, config, fixtureDateWindow));
    } catch (error) {
  ...
  for (const bundle of leagueResults) {
  ...
  const remainingValues = [liveResult.headers?.dailyRemaining, ...leagueResults.flatMap((bundle) => bundle.rateHints)].filter((value) => Number.isFinite(value));
  const payload = {
    matches,
    meta: {
      ...
      errors,
      reason: matches.length ? "ok" : leagueResults.length ? "no_matches_returned" : "all_leagues_failed"
    }
  };
...
async function fetchLeagueBundle(league, config, fixtureDateWindow) {
  ...
  for (const date of fixtureDateWindow) {
    ...
    if (fixturesResult.response.length > 0 && oddsDates.length < config.oddsDays) {
      oddsDates.push(date);
    }
  }
  ...
  for (const date of oddsDates) {
    ...
    for (const oddsEntry of oddsResult.response) {
      oddsByFixture.set(oddsEntry.fixture.id, normalizeOddsEntry(oddsEntry));
    }
  }
...
async function fetchPaginatedOdds(leagueId, season, date, config) {
  ...
  while (page <= totalPages && page <= config.maxOddsPages) {
...
async function apiFetch(path, params, config, tag) {
  await throttleRequests(config.requestSpacingMs);
...
async function throttleRequests(spacingMs) {
  const now = Date.now();
  const waitMs = Math.max(0, nextRequestAt - now);
  nextRequestAt = Math.max(nextRequestAt, now) + spacingMs;
  if (waitMs > 0) {
...
function normalizeFixture(fixtureEntry, odds) {
  ...
  return {
    ...
    sport: "Futbol",
    competition: formatCompetitionName(fixtureEntry.league?.id, fixtureEntry.league?.name),
...
function formatCompetitionName(leagueId, fallbackName) {
  const names = {
  ...
  return names[Number(leagueId)] ?? fallbackName ?? "Competencia";
}

...
export async function fetchFootballFeed() {
  ...
  const bundles = await Promise.allSettled(
    config.leagues.map((league) => fetchLeagueBundle(league, config))
  );
  ...
  for (const bundle of successful) {
    for (const [fixtureId, odds] of bundle.oddsByFixture.entries()) {
...
async function fetchLeagueBundle(league, config) {
  const season = resolveSeason(league.seasonMode, config.targetDate);
  const fixturesResult = await apiFetch(
    "/fixtures",
    { league: String(league.id), season: String(season), date: config.targetDate, timezone: config.timezone },
    config,
    `fixtures:${league.id}`
  );

  const oddsResult = await fetchPaginatedOdds(league.id, season, config);
  const oddsByFixture = new Map();
  ...
  return {
    fixtures: fixturesResult.response,
    oddsByFixture,
    rateHints: [fixturesResult.headers.dailyRemaining, oddsResult.headers.dailyRemaining]
  };
}

async function fetchPaginatedOdds(leagueId, season, config) {
  let page = 1;
  let totalPages = 1;
  const response = [];
  let headers = {};
  ...
  while (page <= totalPages) {
    const result = await apiFetch(
      "/odds",
      { league: String(leagueId), season: String(season), date: config.targetDate, page: String(page) },
      config,
      `odds:${leagueId}:page:${page}`
    );
...
async function apiFetch(path, params, config, tag) {
  ...
  if (!response.ok || data?.errors?.length) {
    const detail = data?.errors?.join("|") || data?.message || response.status;
